    ...
    while(k<max_k){
        ...
        for(unsigned int ck=0; ck<num_prop_chunks; ck++){
            ...
            while((my_filled_emb<max_emb) && (my_k<max_k)) {
                ...
                // they all do the same thing, so enough for the first to update
                // the global state
                if (ck==0){ 
            ...
            // they all do the same thing, so enough for the first to update the
            // global state
            if (ck==0){ 
                k=my_k;
            }
...
std::vector<double> su::stripes_to_condensed_form(su::StripeMap & stripes,
    ...
    for(unsigned int stripe = start; stripe < stop; stripe++){
        ...
        for(uint64_t k = 0; k < n; k++, i++, j++){
            ...
            // (i, j)
            // based on https://docs.scipy.org/doc/scipy/reference/generated/scipy.spatial.distance.squareform.html
