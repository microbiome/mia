...
#' where \eqn{x} is a single value of data.
#' 
#' \item 'log-ratio': Log-ratio of relative abundances. This transformation first
#' converts counts to relative abundances and then applies a logarithmic
#' transformation.
#' \deqn{\log(x/y)}{log(x / y)}
#' where \eqn{x} and \eqn{y} are relative abundances of two features.
#'
...
.transform_assay <- function(
        ...
        method = c(
            ...
            "hellinger", "log", "log10", "log2", "log-ratio", "max", "normalize",
    ...
    if( method %in% c("log10", "log2", "css") ){
        transformed_table <- .apply_transformation(
            assay, method, MARGIN, ...)
    } else if (method %in% c("log-ratio")) {
      transformed_table <- .apply_transformation_logratio(
        assay, method, x = x, ...)
...
###################### .apply_transformation_logratio ########################
# This function computes all pairwise log-ratios (log(x/y)) from relative 
# abundances using a log-transformation with pseudocount.
.apply_transformation_logratio <- function(
    mat, method, x, pseudocount = 1e-6, ...) {
  
  # Add pseudocount to avoid division by zero and log(0)
  mat_pseudo <- mat + pseudocount
  
  # Convert to relative abundances (row-wise) — taxa sum to 1
  mat_rel <- sweep(mat_pseudo, 1, rowSums(mat_pseudo), FUN = "/")
  
  # Log-transform the relative abundances
  mat_log <- log(mat_rel)
  
  # Get all pairwise combinations of taxa (i.e., rows)
  taxa <- rownames(mat_log)
  combs <- utils::combn(taxa, 2, simplify = FALSE)
  
  # Compute log-ratios: log(x/y) = log(x) - log(y), per sample (columns)
  logratios <- lapply(combs, function(pair) {
    t1 <- pair[1]
    t2 <- pair[2]
    ratio_name <- paste0("logratio_", t1, "_vs_", t2)
    values <- mat_log[t1, ] - mat_log[t2, ]
    matrix(values, nrow = 1, dimnames = list(ratio_name, colnames(mat_log)))
  })
  
  # Combine into matrix
  result <- do.call(rbind, logratios)
  
  # Attach metadata
  attr(result, "mia") <- "log-ratio"
  attr(result, "parameters") <- list(pseudocount = pseudocount)
