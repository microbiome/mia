...
#' a <- getPrevalentTaxa(enterotype, 1/100, 50/100)
#' @export
#' @references 
...
#' @author Leo Lahti 
#' @keywords utilities
getPrevalentTaxa <- function(x, detection=1/100, prevalence=50/100,
    ...
    if (include.lowest) {
        taxa <- names(which(prevalence(x, detection,
            include.lowest=include.lowest, as_relative=FALSE) >= prevalence))
    } else {
        taxa <- names(which(prevalence(x, detection,
        include.lowest=include.lowest, as_relative=FALSE) > prevalence))
    }
