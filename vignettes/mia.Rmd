...
uses specific columns in the `rowData` of a `SummarizedExperiment` object.
`taxonomyRanks` can be used to construct a `character` vector of available 
taxonomic levels. This can be used for subsetting for example.
...
Keep in mind, that rows with `NA` or similar value (via the `empty.fields`
argument) will be removed, if you set `na.rm = TRUE`. Depending on this
different number of rows will be returned.
...
with microbiome data. `transformCounts` can be used for this and offers a few
...
```{r}
altExp(se,"Genus") <- runMDS2(altExp(se,"Genus"), 
                              FUN = vegan::vegdist,
                              method = "bray",
                              name = "BrayCurtis", 
                              ncomponents = 5, 
