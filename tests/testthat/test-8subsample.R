...
test_that("rarefyAssay", {
    ...
    expect_equal(ncol(tse.subsampled), 26)
    # check number of features removed is correct
    expnFeaturesRemoved <- 6611
    ...
    #obsSampleRemoved <- colnames(GlobalPatterns)[!colnames(GlobalPatterns) %in% colnames(tse.subsampled)] # None was removed
    #expect_equal(obsSampleRemoved, expSampleRemoved)
    
    # check if all samples subsampled to even depth
    expColSums <- rep(60000, 26)
    ...
    expFeaturesRemovedRP <- c("951","244423","586076","246140","143239",
                              "244960","144887","141782","215972","31759")
