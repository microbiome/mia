...
test_that("merge", {
    ...
    FUN_check_x <- function(x,archetype=1){
        actual <- agglomerateByVariable("rows", x, f, archetype)
    ...
    merged  <- agglomerateByVariable("rows", tse, f=rowData(tse)$group2, 
                                    mergeTree=TRUE)
    merged2 <- agglomerateByVariable("rows", tse, f = rowData(tse)$group2,
                                    mergeTree = FALSE)
    merged3 <- agglomerateByVariable("rows", esophagus,
                                    f = rowData(esophagus)$group2, 
                                    mergeTree = TRUE)
    merged4 <- agglomerateByVariable("rows", tse, 
                                    merge.by = rowData(tse)$group2, 
                                    mergeTree = TRUE)
    merged5 <- agglomerateByVariable("rows", tse, f = rowData(tse)$group2, 
                                    mergeTree = TRUE)
    ...
    expect_equal(agglomerateByVariable("rows", tse, f=rowData(tse)$group2), 
                agglomerateByVariable("rows", tse, f=rowData(tse)$group2))
    
    # Both datasets have group variable
    merged <- agglomerateByVariable("rows", tse, f = rowData(tse)$group, 
                                    mergeTree = TRUE)
    merged2 <- agglomerateByVariable("rows", tse, f = rowData(tse)$group, 
                                     mergeTree = FALSE)
    expect_equal( rowLinks(merged)$whichTree, 
                  rowLinks(merged2)$whichTree )
    expect_false( all(rowLinks(merged) == rowLinks(merged2)) )
