...
test_that("Unifrac beta diversity", {
    ...
    expect_error(
        getUnifrac(tse, assay.type = "test", tree.name = "phylo",
                         weighted = FALSE)
    )
    expect_error(
        getUnifrac(tse, assay.type = 2, tree.name = "phylo",
                         weighted = FALSE)
    )
    expect_error(
        getUnifrac(tse, assay.type = TRUE, tree.name = "phylo",
                         weighted = FALSE)
    )
    expect_error(
        getUnifrac(tse, assay.type = "counts", tree.name = "test",
                         weighted = FALSE)
    )
    expect_error(
        getUnifrac(tse, assay.type = "counts", tree.name = 1,
                         weighted = FALSE)
    )
    expect_error(
        getUnifrac(tse, assay.type = "counts", tree.name = TRUE,
                         weighted = "FALSE")
    )
    expect_error(
        getUnifrac(tse, assay.type = "counts", tree.name = "phylo",
                         weighted = 1)
    )
    ...
    unifrac_mia <- as.matrix(getUnifrac(tse, weighted = FALSE))
    ...
    unifrac_mia <- as.matrix(getUnifrac(tse, weighted = TRUE))
    ...
    # Calculate unweighted unifrac
    expect_warning(
    unifrac_mia <- getUnifrac(tse, weighted = FALSE, tree.name = "tree2")
    )
    ...
    expect_warning(
    unifrac_mia <- getUnifrac(tse, weighted = TRUE, tree.name = "tree2")
    )
    unifrac_mia <- as.matrix(unifrac_mia)
