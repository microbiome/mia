...
test_that("estimateDominance", {
    ...
    test_internal_estimateDominance <- function(tse){
        ...
        expect_equal(round(as.vector(mia:::.calc_core_dominance(tse)), 7),
                     round(c(0.9605911, 0.8980392, 0.9086758)), 7)

        #.get_dominance
        #Rounded because, without it gave an error (average difference was
        #about 1E-08). The accuracy is sufficient to test if working correctly
        expect_equal(as.vector(mia:::.calc_dominance(assays(tse)$counts,
                                                     index="absolute",
                                                     ntaxa = 1,
                                                     aggregate = TRUE)),
                     c(52, 42, 124))

        expect_equal(round(as.vector(mia:::.calc_dominance(assays(tse)$counts,
                                                           index="relative",
                                                           ntaxa = 1,
                                                           aggregate = TRUE)), 7),
