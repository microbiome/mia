...
test_that("transformAssay", {
    testTransformations <- function(tse){
        ...
        ########################### LOG-RATIO ######################################
        # Test that log-ratio transformation works correctly on a small subset
        data("GlobalPatterns")
        tse <- GlobalPatterns
        message("[TEST] Running log-ratio transformation on small subset...")
        
        # Use a small subset of GlobalPatterns for faster testing
        tse_small <- tse[1:5, 1:4]  # 5 taxa, 4 samples
        
        # Apply log-ratio transformation
        tse_small <- transformAssay(tse_small, method = "log-ratio", pseudocount = 1e-6)
        
        # Check if the 'log-ratio' assay exists
        expect_true("log-ratio" %in% altExpNames(tse_small))
        
        # Extract the transformed assay
        logr <- assay(altExp(tse_small, "log-ratio"))
        
        # Expect correct number of log-ratio features (combinations of taxa)
        expected_nrows <- choose(nrow(tse_small), 2)
        expect_equal(nrow(logr), expected_nrows)
        
        # Expect same number of columns as original data
        expect_equal(ncol(logr), ncol(tse_small))
        
        # Expect numeric values only, no NA or Inf
        expect_type(logr, "double")
        expect_false(any(is.na(logr)))
        expect_false(any(is.infinite(logr)))
        
        # Expect correct metadata attributes
        expect_equal(attr(logr, "mia"), "log-ratio")
        expect_equal(attr(logr, "parameters")$pseudocount, 1e-6)
    }
